angular.module('bc.bitcoin-address', []).service('BitcoinAddress', [function () {
  ...
  function initPositions (BigNumber) {
  ...
  this.validate = function (BigNumber, address, address_type) {
    // Init
    initPositions(BigNumber);

    // default is to check that address is regular production address
    address_type = address_type || 'prod';

    try {
      var decoded_hex = base58_decode(BigNumber, address);
    } catch (e) {
  ...
  function base58_decode(BigNumber, payload) {
